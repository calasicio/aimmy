...
bool Window::CreateDevice()
{
  ...
  if (result == DXGI_ERROR_UNSUPPORTED)
  {
    ...
    logger::fatal("DXGI_ERROR | Created with D3D_DRIVER_TYPE_WARP");
  }

  if (result != S_OK)
  {
    logger::fatal("Device Not Okay");
    return false;
  }
...
void Window::DestroyDevice()
{
  if (device)
  {
    ...
    logger::warning("Device Not Found to destroy.");
}
...
bool Window::SpawnWindow()
{
  ...
  {
    logger::fatal("Failed to set layered window attributes");
    return false;
  }
  ...
  logger::info("Window Created with HWND " + std::to_string((uintptr_t)hwnd) + " dimensions " + std::to_string(width) + "w " + std::to_string(height) + "h");
  return true;
}
...
bool Window::CreateImGui()
{
  ...
  {
    logger::fatal("Failed ImGui_ImplWin32_Init");
    return false;
  }
  ...
  {
    logger::fatal("Failed ImGui_ImplDX11_Init");
    return false;
  }
...
bool Window::SetAffinity(HWND window, WindowAffinity afi)
{
  ...
  if (status)
    logger::info("Set Window Affinity to " + mode_str);
  else
    logger::fatal("Failed to set Window Affinity to " + mode_str);
...
LRESULT CALLBACK window_procedure(HWND window, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_KEYUP:
  {
    ...
    if (toggle_key)
    {
      ...
      logger::info("Toggle key has been captured, toggling renderMenu state");
    }
    break;
  }
  case WM_DESTROY:                                                // We dont handle this event
    ...
    break;
  case WM_CLOSE:
    logger::info("Window procedure WM_CLOSE event triggered");
  ...
  case WM_SIZE:
    ...
    return 0;
  case WM_DPICHANGED:
    logger::info("Window procedure WM_DPICHANGED event triggered");
